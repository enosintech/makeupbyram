...
const Contact = ({ scrollPosition }) => {
    ...
    useGSAP(() => {
        ...
        pinAnimations(contactRef.current, "top top", () => "+=" + contactRef.current.offsetHeight, false, true);

        pinAnimations(lastPinRef.current, "top top", () => "+=" + lastPinRef.current.offsetHeight * 2, false, true);
  ...
  return (
    <section style={{height: window.innerHeight * 3}} className="w-full bg-black -mt-2 flex flex-col relative z-10 overflow-x-hidden">
        ...
        <div ref={lastPinRef} style={{height: window.innerHeight}}  className="w-full lastPin flex items-center justify-center relative whiteScrubTrigger sm:border-none border-b-4 border-white">
            <div ref={contactOverlayRef} style={{height: window.innerHeight}}  className="contactOverlay w-[100vw] fixed z-50 left-0 top-0 flex flex-col items-center justify-center gap-y-4">
